...
def _edist(p, q):
    print(Levenshtein_Distance(p, q))
    return Levenshtein_Distance(p, q)

def _eps_neighborhood(p,q,eps):
	# return _dist(p,q) < eps
    return _edist(p, q) < eps
...
def test_custom_dbscan():
    m = np.array([["aa", "aaa"], ["aa", "aaa"]])
    eps = 3
    min_points = 2
